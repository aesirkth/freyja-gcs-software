...
def build(parent: int | str) -> dict:
    with dpg.child_window(parent=parent, tag=PLOT_DASHBOARD["velocity:panel"], width=0, height=0, border=False):
        with dpg.plot(height=400, width=-1) as plot_id:
            ...
            line_id = dpg.add_scatter_series(xs, ys, label="Position", parent=y_axis, tag=PLOT_DASHBOARD["plot_temp"])
...
def update(data: TelemetryInput, tags: dict) -> None:
    dpg.set_value(tags["plot_temp"], [[data.east_enu], [data.north_enu]])
